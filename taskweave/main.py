...
from taskweave.agents.providers.registry import ProviderRegistry
from taskweave.agents.subagent_orchestrator import SubagentOrchestrator, create_orchestrator
from taskweave.channels.cli_channel import CLIChannel
from taskweave.config.loader import load_config
from taskweave.config.settings import Settings, ExecutionMode
from taskweave.dispatcher.subagent_registry import SubagentRegistry, GuardrailConfig
from taskweave.dispatcher.event_bus import EventBus, TurnLogStore
from taskweave.dispatcher.dispatcher import Dispatcher
from taskweave.gateway.server import GatewayServer
from taskweave.memory.embeddings import EmbeddingGenerator
from taskweave.memory.hybrid_store import HybridMemoryStore
from taskweave.memory.sqlite_store import SQLiteMemoryStore
from taskweave.memory.vector_store import VectorMemoryStore
from taskweave.sessions.manager import SessionManager
from taskweave.tools.loader import load_builtin_tools, load_tool_dirs
from taskweave.tools.registry import ToolRegistry
from taskweave.tools.skill_md_loader import SkillManager
from taskweave.tools.builtin.load_skill_tool import LoadSkillTool, set_skill_manager
from taskweave.utils.logging import setup_logging, get_logger
...
async def bootstrap_master_subagent(config: Settings) -> dict:
    ...
    # 5. 基础 Agent
    from taskweave.tools.registry import ToolRegistry
    from taskweave.tools.builtin.git_resolve_conflict import GitResolveConflictTool
...
async def _init_tools(config: Settings, logger) -> ToolRegistry:
    ...
    from taskweave.config.settings import PROJECT_ROOT
...
async def bootstrap(config: Settings) -> dict:
    ...
    logger.info("正在初始化 TaskWeave v0.2.0...")
...
async def run(config_path: str | None = None) -> None:
    ...
    try:
        ...
        if "cli" in config.channels:
            ...
            logger.info("TaskWeave 正在运行... (Ctrl+C 退出)")
        ...
        await memory.close()
        logger.info("TaskWeave 已关闭")


def main():
    """命令行入口"""
    parser = argparse.ArgumentParser(description="TaskWeave — AI 助手平台")
