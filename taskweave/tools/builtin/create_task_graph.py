...
class PendingGraphStore:
    ...
    每个 Orchestrator 创建自己的 store 并绑定到 CreateTaskGraphTool 实例。
    DAG 执行完后调度器显式 pop。

    持久化（可选）：
      - 构造时传 db_path 即可启用 SQLite 落地
      - put() 同时写内存和 DB
      - pop() 同时删内存和 DB
      - initialize() 从 DB 恢复进程崩溃前未执行的请求
      - close() 关闭 DB 连接

    内存优先：get() 始终读内存（同步），DB 仅用作持久化镜像。
    db_path=None 时退化为纯内存模式（兼容老调用）。
    """
    ...
    async def initialize(self) -> None:
        """打开 DB 连接 + 建表 + 恢复未消费的请求。

        如果 db_path 为 None 直接返回（纯内存模式）。
        """
        ...
        await self._db.execute("""
            CREATE TABLE IF NOT EXISTS pending_graphs (
        ...
        # 恢复：把上次进程留下的未消费请求加载到内存。这是 Phase 1
        # "断点恢复" 的最后一公里 —— Master 拿到 graph_id 后崩溃时，
        # 重启可以继续 await 它的执行结果（前提是 master 路径同样恢复）。
        cur = await self._db.execute(
            "SELECT graph_id, request_json FROM pending_graphs"
        )
        ...
        for row in rows:
            try:
                ...
                req = TaskGraphRequest.model_validate(data)
                self._store[row[0]] = req
                recovered += 1
            except Exception as e:
                logger.warning("恢复 pending graph %s 失败: %s", row[0], e)
        if recovered:
            logger.info(
                "从 %s 恢复了 %d 个未消费的 task graph 请求",
                self._db_path, recovered,
            )
    ...
    async def put(self, graph_id: str, request: TaskGraphRequest) -> None:
        """写入：先内存（即时可见），再 DB（持久化）。"""
        self._store[graph_id] = request
        if self._db is not None:
            try:
                await self._db.execute(
                    "INSERT OR REPLACE INTO pending_graphs"
                    "(graph_id, request_json, created_at) VALUES(?,?,?)",
                    (
                        graph_id,
                ...
                # 持久化失败不能阻塞执行，但要明确告警
                logger.error(
                    "pending graph 持久化失败（已写内存，DB 写入失败）: %s — %s",
                    graph_id, e,
                )
    ...
    async def pop(self, graph_id: str) -> TaskGraphRequest | None:
        """删除：先内存，再 DB。"""
        item = self._store.pop(graph_id, None)
        if self._db is not None:
            try:
                await self._db.execute(
                    "DELETE FROM pending_graphs WHERE graph_id = ?",
                    (graph_id,),
                )
                await self._db.commit()
            except Exception as e:
                logger.warning(
                    "pending graph 从 DB 删除失败（内存已清）: %s — %s",
                    graph_id, e,
                )
        return item
...
async def clear_pending_graph(graph_id: str) -> None:
    """[兼容 API] 清除默认 store 的 request（异步以兼容持久化 store）"""
    await _default_store.pop(graph_id)
                        ...
                        "properties": {
                            ...
                            "custom_role_config": {
                                ...
                                "properties": {
                                    ...
                                    "requires_worktree": {
                                        "type": "boolean",
                                        "description": "是否需要 Git Worktree 物理隔离（写代码角色必须 true；只读检索/规划类设 false）",
                                    },
                                    "requires_validation": {
                                        "type": "boolean",
                                        "description": (
                                            "提交前是否必须先通过 run_linter / run_tests 验证。"
                                            "**写代码的角色（产出 .py/.js 等代码）应设 true**，"
                                            "门禁会强制 PASSED 才放行 submit。"
                                            "**不写代码的角色（只读检索 / 设计规划 / 代码审查）应设 false**，"
                                            "否则它们没有可被验证的产物，会卡在门禁里。"
                                            "未显式设置时，系统按 allowed_tools 是否含 write_file/edit_file 智能推断。"
                                        ),
                                    },
                                    "max_steps": {
                                        "type": "integer",
        ...
        try:
            ...
            if isinstance(tasks_data, str):
                try:
                    ...
                    # 附带异常位置 + native 范例，引导 LLM 一次纠正而不是原样重发。
                    return (
                        "错误: tasks 参数无法解析为 JSON 数组。\n"
                        f"  解析异常: {e.msg} at pos {e.pos}\n"
                        "  推荐写法: 直接把 tasks 作为数组传入，不要序列化成字符串。\n"
                        "  正确格式示例:\n"
                        '  {"tasks": [\n'
                        '    {"id": "t1", "role": "CoderAgent", "instruction": "..."},\n'
                        '    {"id": "t2", "role": "TesterAgent", "instruction": "...", '
                        '"depends_on": ["t1"]}\n'
                        "  ]}"
                    )
            if not tasks_data:
                return (
                    "错误: tasks 参数为空。请传入至少一个任务节点，"
                    '格式参考: [{"id": "t1", "role": "CoderAgent", "instruction": "..."}]'
                )
            ...
            # 存储待执行的任务图（持久化，供调度器使用 + 进程崩溃恢复）
            await self._store.put(result.graph_id, request)
