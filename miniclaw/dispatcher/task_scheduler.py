...
class TaskScheduler:
    ...
    async def _execute_with_agent(
        ...
        if subagent_config.role_name == "CleanupAgent":
            from miniclaw.tools.builtin.delete_artifact import (
                DeleteArtifactTool,
        ...
        if original_submit:
            ...
            from miniclaw.agents.critic.submit_claim_checker import (
    ...
    def _reconcile_outcomes_into_result(self, ctx: GraphExecutionContext) -> None:
        ...
        在 _execute_levels 完成后、status 判定前调用一次：
          ...
          3. 反向：outcome 显示成功但仍在 failed_tasks 的（极罕见，merge 异常恢复）
             也尊重 outcome
        """
        from miniclaw.types.structured_result import TaskStatus
        result = ctx.result
